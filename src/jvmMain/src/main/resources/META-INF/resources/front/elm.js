...
var $author$project$Main$LiveStatus = function (a) {
	return {$: 'LiveStatus', a: a};
};
var $author$project$Main$apiUrl = 'http://localhost:8080/';
...
var $author$project$Main$checkLive = function (_v0) {
	return $elm$http$Http$get(
		{
			expect: $elm$http$Http$expectString($author$project$Main$LiveStatus),
			url: $author$project$Main$apiUrl + '/live'
		});
};
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{comments: _List_Nil, log: _List_Nil, status: $author$project$Main$Disconnected},
		$author$project$Main$checkLive(_Utils_Tuple0));
};
var $author$project$Main$NewCommentsRaw = function (a) {
	return {$: 'NewCommentsRaw', a: a};
};
...
var $author$project$Main$subscriptions = function (_v0) {
	return $author$project$Main$messageReceiver($author$project$Main$NewCommentsRaw);
};
...
var $author$project$Main$handleHttpError = F2(
	function (model, http) {
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					log: _Utils_ap(
						...
						_List_fromArray(
							[
								$author$project$Main$errorToString(http)
							]))
				}),
			$elm$core$Platform$Cmd$none);
	});
var $elm$json$Json$Decode$list = _Json_decodeList;
var $author$project$Main$LiveEvent = F2(
	function (eventType, comment) {
		return {comment: comment, eventType: eventType};
	});
var $author$project$Main$CommentToValidate = F7(
	function (id, commentId, user, item, timeStamp, fullComment, action) {
		return {action: action, commentId: commentId, fullComment: fullComment, id: id, item: item, timeStamp: timeStamp, user: user};
	});
...
var $author$project$Main$commentDecoder = A8(
	$elm$json$Json$Decode$map7,
	$author$project$Main$CommentToValidate,
	A2($elm$json$Json$Decode$field, 'id', $elm$json$Json$Decode$string),
	...
	A2($elm$json$Json$Decode$field, 'item', $elm$json$Json$Decode$int),
	A2($elm$json$Json$Decode$field, 'timestamp', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'fullComment', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'action', $elm$json$Json$Decode$string));
var $author$project$Main$liveEventDecoder = A3(
	$elm$json$Json$Decode$map2,
	$author$project$Main$LiveEvent,
	A2($elm$json$Json$Decode$field, 'eventType', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'comment', $author$project$Main$commentDecoder));
var $author$project$Main$liveEventsDecoder = $elm$json$Json$Decode$list($author$project$Main$liveEventDecoder);
var $author$project$Main$sendMessage = _Platform_outgoingPort('sendMessage', $elm$json$Json$Encode$string);
var $author$project$Main$LiveStarted = function (a) {
	return {$: 'LiveStarted', a: a};
};
var $author$project$Main$LiveStopped = function (a) {
	return {$: 'LiveStopped', a: a};
};
var $elm$http$Http$stringBody = _Http_pair;
var $author$project$Main$toggleLive = function (status) {
	return (!_Utils_eq(status, $author$project$Main$Connected)) ? $elm$http$Http$request(
		{
			body: A2($elm$http$Http$stringBody, 'application/x-www-form-urlencoded', 'video=fb-123&liveId=Live-2022-07-17&items=100'),
			expect: $elm$http$Http$expectString($author$project$Main$LiveStarted),
			headers: _List_Nil,
			method: 'POST',
		...
		}) : $elm$http$Http$request(
		{
			...
			expect: $elm$http$Http$expectString($author$project$Main$LiveStopped),
			headers: _List_Nil,
			method: 'DELETE',
			timeout: $elm$core$Maybe$Nothing,
			tracker: $elm$core$Maybe$Nothing,
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'NewCommentsRaw':
				var rawComments = msg.a;
				var _v1 = A2($elm$json$Json$Decode$decodeValue, $author$project$Main$liveEventsDecoder, rawComments);
				if (_v1.$ === 'Ok') {
					var liveEvents = _v1.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
			...
			case 'AuthorToClipboard':
				var name = msg.a;
				return _Utils_Tuple2(
					model,
					$author$project$Main$clipboardCopy(name));
					...
					_Utils_update(
						model,
						{
							comments: A2(
								...
								function (c) {
									return !_Utils_eq(comment.id, c.id);
								},
								model.comments)
						}),
					$author$project$Main$sendMessage(comment.id));
			case 'Switch':
				return _Utils_Tuple2(
					model,
					$author$project$Main$toggleLive(model.status));
			case 'LiveStatus':
				var result = msg.a;
				if (result.$ === 'Ok') {
					...
					if ((http.$ === 'BadStatus') && (http.a === 404)) {
						...
						return A2($author$project$Main$handleHttpError, model, http);
					}
				}
			case 'LiveStarted':
				var result = msg.a;
...
var $author$project$Main$AuthorToClipboard = function (a) {
	return {$: 'AuthorToClipboard', a: a};
};
var $author$project$Main$CommentValidated = function (a) {
	return {$: 'CommentValidated', a: a};
};
...
var $author$project$Main$articleFromComment = function (comment) {
	return A2(
			...
			[
				...
				_List_fromArray(
					[
						...
						A4($rtfeldman$elm_css$Css$rgba, 51, 51, 51, 0.23)),
						...
						_List_fromArray(
							[
								...
								$rtfeldman$elm_css$Css$px(47),
								$rtfeldman$elm_css$Css$px(11),
								A4($rtfeldman$elm_css$Css$rgba, 51, 51, 51, 0.18))
							]))
					]))
			]),
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						$author$project$Main$AuthorToClipboard(comment.user.name)),
						$rtfeldman$elm_css$Html$Styled$Attributes$css(
						_List_fromArray(
							[
								$rtfeldman$elm_css$Css$color(
								$rtfeldman$elm_css$Css$hex('#979cb0')),
								$rtfeldman$elm_css$Css$fontWeight(
								$rtfeldman$elm_css$Css$int(600)),
								$rtfeldman$elm_css$Css$fontSize(
								$rtfeldman$elm_css$Css$px(20)),
								$rtfeldman$elm_css$Css$letterSpacing(
								$rtfeldman$elm_css$Css$px(0.64)),
								$rtfeldman$elm_css$Css$hover(
								_List_fromArray(
									[
										$rtfeldman$elm_css$Css$backgroundColor(
										$rtfeldman$elm_css$Css$hex('#e6e5d9'))
									]))
							]))
					]),
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(comment.user.name)
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$span,
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$Events$onClick(
						$author$project$Main$CommentValidated(comment)),
						$rtfeldman$elm_css$Html$Styled$Attributes$css(_List_Nil)
					]),
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(comment.action)
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$span,
				_List_Nil,
				_List_fromArray(
					[
						...
						$elm$core$String$fromInt(comment.item))
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$span,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$text(comment.fullComment)
							]))
					]))
			]));
};
...
var $author$project$Main$displayListOfComments = function (model) {
	return A2(
		...
		A2(
			$elm$core$List$map,
			function (c) {
				return $author$project$Main$articleFromComment(c);
			},
			model.comments));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$Events$onClick($author$project$Main$Switch)
					]),
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(
						_Utils_eq(model.status, $author$project$Main$Connected) ? 'Stopper Live' : 'Démarrer Live')
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$div,
				_List_Nil,
				...
				$author$project$Main$displayListOfComments(model)
			]));
};
